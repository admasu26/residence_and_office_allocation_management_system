...
// Fetch approved allocation requests ordered by office type (private first) then rank
$sql = "SELECT * FROM office_allocation_requests 
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['allocate'])) {
        if (!empty($_POST['selected_requests'])) {
            ...
            foreach ($selected_requests as $request_id) {
                ...
                $request_sql = "SELECT * FROM office_allocation_requests WHERE id = ?";
                ...
                if ($request) {
                    ...
                    $allocated_to_name = $request['name'];
                    $allocated_to_username = $request['username'];

                    // Find an available resource
                    $resource_sql = "SELECT * FROM office_resource 
                                    WHERE resource_type = ? AND campus = ? AND status = 'Available' 
                                    LIMIT 1";
                    $stmt = $conn->prepare($resource_sql);
                    $stmt->bind_param("ss", $office_type, $campus);
                    $stmt->execute();
                    ...
                    if ($resource) {
                        ...
                        $allocate_sql = "UPDATE office_resource SET status = 'Allocated' WHERE id = ?";
                        ...
                        $update_request_sql = "UPDATE office_allocation_requests SET status = 'Allocated' WHERE id = ?";
                        ...
                        $insert_allocation_sql = "INSERT INTO office_allocation 
                                                (request_id, resource_id, allocated_to_name, allocated_to_username, campus, building, floor, room_number) 
                                                VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                        $stmt = $conn->prepare($insert_allocation_sql);
                        $stmt->bind_param("iissssii", $request_id, $resource['id'], $allocated_to_name, 
                                        $allocated_to_username, $resource['campus'], $resource['building'], 
                                        $resource['floor'], $resource['room_number']);
                        $stmt->execute();
                        $stmt->close();

                        // Send notification
                        $notification_message = "Your office allocation request has been successfully processed.";
    ...
    elseif (isset($_POST['deallocate'])) {
        if (!empty($_POST['selected_allocations'])) {
            foreach ($_POST['selected_allocations'] as $allocation_id) {
                // Get allocation details
                $allocation_sql = "SELECT * FROM office_allocation WHERE id = ?";
                ...
                if ($allocation) {
                    ...
                    $update_resource_sql = "UPDATE office_resource SET status = 'Available' WHERE id = ?";
                    ...
                    $update_request_sql = "UPDATE office_allocation_requests SET status = 'Approved' WHERE id = ?";
                    $stmt = $conn->prepare($update_request_sql);
                    $stmt->bind_param("i", $allocation['request_id']);
                    $stmt->execute();
                    $stmt->close();

                    // Delete the allocation record
                    $delete_sql = "DELETE FROM office_allocation WHERE id = ?";
                    ...
                    // Send notification
                    $notification_message = "Your office allocation has been deallocated.";
                    $notification_sql = "INSERT INTO notifications (username, message) VALUES (?, ?)";
                    $stmt = $conn->prepare($notification_sql);
    ...
    header("Location: office_allocation.php");
    exit();
}

// Fetch allocation details
$allocation_sql = "SELECT a.id as allocation_id, a.request_id, a.allocated_to_name, a.allocated_to_username, 
                          a.campus, a.building, a.floor, a.room_number, 
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="container">
            <h1><i class="fas fa-building"></i> Office Allocation</h1>
                            ...
                            <table class="table table-striped table-hover">
                                <thead>
                                    <tr>
                                        ...
                                        <th><i class="fas fa-home"></i> Office Type</th>
            ...
            <div id="allocationDetailsSection" class="view-section hidden-section">
                <div class="section-card">
                    ...
                    <form method="POST" action="">
                                ...
                                <tbody>
                                    ...
                                    if ($allocation_result->num_rows > 0) {
                                        while ($row = $allocation_result->fetch_assoc()) {
                                            echo "<tr>
                                                    <td><input type='checkbox' name='selected_allocations[]' value='" . $row["allocation_id"] . "'></td>
                        ...
                        <div class="action-buttons">
                            <button type="submit" name="deallocate" class="btn btn-danger">
