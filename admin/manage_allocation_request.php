...
// Fetch allocation requests based on filter, ordered by preferred type (single first) then rank
$sql = "SELECT * FROM allocation_requests WHERE status = ? 
        ORDER BY 
        CASE WHEN prefered_type = 'three bed room' THEN 1 
             WHEN prefered_type = 'two bed room' THEN 2 
             ELSE 3 END,
        rank ASC";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['rank_score'])) {
    ...
    $pending_sql = "SELECT * FROM allocation_requests WHERE status = 'Pending'";
    ...
    while ($row = $pending_result->fetch_assoc()) {
        ...
        $update_sql = "UPDATE allocation_requests SET score = ? WHERE id = ?";
    ...
    // Rank single and shared residence types separately
    $residence_types = ['single', 'shared']; // Single first as higher priority
    
    foreach ($residence_types as $type) {
        // Get requests for this residence type ordered by score
        $rank_sql = "SELECT id, score FROM allocation_requests 
                    WHERE status = 'Pending' AND prefered_type = ?
                    ORDER BY score DESC";
        ...
        while ($rank_row = $rank_result->fetch_assoc()) {
            ...
            $update_rank_sql = "UPDATE allocation_requests SET rank = ? WHERE id = ?";
    ...
    header("Location: manage_allocation_request.php");
    exit();
}
    ...
    if (isset($_POST['update_status_single'])) {
        ...
        if ($status !== $current_status) {
            ...
            if ($status === 'Rejected' || $status === 'Approved') {
                ...
                $fetch_username_sql = "SELECT username FROM allocation_requests WHERE id = ?";
                ...
                $notification_message = "Your residence allocation request has been " . strtolower($status) . ".";
        ...
        header("Location: manage_allocation_request.php?status=" . urlencode($status_filter));
        exit();
    }
        ...
        if (!empty($_POST['selected_requests'])) {
            ...
            foreach ($selected_requests as $id) {
                ...
                if ($status === 'Rejected') {
                    ...
                    // Insert a notification into the `notifications` table
                    $notification_message = "Your residence allocation request has been rejected.";
                    $notification_sql = "INSERT INTO notifications (username, message) VALUES (?, ?)";
                    $stmt = $conn->prepare($notification_sql);
                    $stmt->bind_param("ss", $username, $notification_message);
                    $stmt->execute();
...
<head>
    ...
    <title>Manage Residence Allocation Requests</title>
...
<body>
    ...
    <div class="main-content">
        <div class="container-fluid">
            <h2 class="mb-4">
                <i class="fas fa-tasks"></i> Manage Residence Allocation Requests
            </h2>
            <?php
                    ...
                    <table class="table table-striped table-hover">
                        ...
                        <tbody>
                            ...
                            if ($result->num_rows > 0) {
                                ...
                                echo "<tr><td colspan='15' class='text-center'>No " . strtolower($status_filter) . " allocation requests found.</td></tr>";
                            }
                            ?>
                        </tbody>
                    </table>
