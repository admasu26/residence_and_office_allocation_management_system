...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['generate_report'])) {
    ...
    switch ($report_type) {
        ...
        case 'track_resource':
            $sql = "SELECT campus, building, floor, room_number, resource_type, status FROM resources ORDER BY campus, building, floor, room_number";
            $result = $conn->query($sql);
            if ($result->num_rows > 0) {
                $report_data = $result->fetch_all(MYSQLI_ASSOC);
            } else {
    ...
    $sql = "INSERT INTO reports (title, data) VALUES (?, ?)";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['send_report'])) {
    ...
    $sql = "UPDATE reports SET sent_to_director = TRUE WHERE id = ?";
    $stmt = $conn->prepare($sql);
...
<body>
    ...
    <div class="sidebar">
        ...
        <ul>
            ...
            <li><a href="send_report.php"><i class="fas fa-id-card"></i> Generate Report</a></li>
        </ul>
    </div>
    ...
    <div class="main-content">
        <div class="container">
            <h1><i class="fas fa-file-alt"></i> Generate Residence Report</h1>
            ...
            <?php if (isset($_SESSION['report_data']) && !empty($_SESSION['report_data'])): ?>
                <div class="mt-4">
                    ...
                    <form method="POST" action="send_report.php" class="mt-3">
