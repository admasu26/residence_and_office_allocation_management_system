    ...
    if (isset($_POST["add_resource"])) {
        ...
        if (!empty($campus) && !empty($building) && !empty($floor) && !empty($room_start) && !empty($room_end) && !empty($resource_type)) {
            ...
            for ($room_number = $room_start; $room_number <= $room_end; $room_number++) {
                ...
                $check_sql = "SELECT * FROM office_resource WHERE campus = ? AND building = ? AND floor = ? AND room_number = ?";
                ...
                if ($result->num_rows > 0) {
                    ...
                    $sql = "INSERT INTO office_resource (campus, building, floor, room_number, resource_type, status) 
                            VALUES (?, ?, ?, ?, ?, 'Available')";
        ...
        if (!empty($id) && !empty($campus) && !empty($building) && !empty($floor) && !empty($room_number) && !empty($resource_type) && !empty($status)) {
            // Update the resource
            $sql = "UPDATE office_resource SET campus = ?, building = ?, floor = ?, room_number = ?, resource_type = ?, status = ? WHERE id = ?";
            $stmt = $conn->prepare($sql);
...
$floors_sql = "SELECT DISTINCT floor FROM office_resource";
...
$resources_sql = "SELECT * FROM office_resource WHERE 1=1";
...
$edit_resource = null;
if (!empty($edit_id)) {
    $edit_sql = "SELECT * FROM office_resource WHERE id = ?";
                ...
                <div class="card-body">
                    ...
                    <form method="POST" action="">
                        ...
                        <div class="mb-3">
                            <label for="resource_type" class="form-label"><i class="fas fa-home"></i> Resource Type:</label>
                            <select name="resource_type" class="form-select" required>
                                <option value="private" <?= $edit_resource['resource_type'] == 'private' ? 'selected' : '' ?>>Private</option>
                                <option value="shared" <?= $edit_resource['resource_type'] == 'shared' ? 'selected' : '' ?>>Shared</option>
                            </select>
                        </div>
                        <div class="mb-3">
